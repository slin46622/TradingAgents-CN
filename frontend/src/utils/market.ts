// 市场参数规范化：将各类板块/交易所/缩写统一为分析模块支持的 A股/美股/港股/加密
export const normalizeMarketForAnalysis = (market: any): string => {
  const raw = String(market ?? '').trim()
  const upper = raw.toUpperCase()
  const cn = raw
  const isCrypto = cn === '加密' || ['CRYPTO', 'CRYPTOCURRENCY', '加密货币'].includes(upper)
  const isA = [
...
export const getMarketByStockCode = (stockCode: string): string => {
  ...
  // 加密货币：字母数字组合，以 USDT/BTC/ETH/BNB/SOL/BUSD 结尾
