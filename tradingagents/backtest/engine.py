...
class EvaluationConfig:
    ...
    commission_rate: float = 0.0003   # 手续费率（双边，默认A股0.03%）
    slippage_rate: float = 0.001      # 滑点（默认0.1%）
...
class PerformanceMetrics:
    ...
    sharpe_ratio: float = 0.0            # 夏普比率（无风险利率2%）
    max_drawdown_pct: float = 0.0        # 最大回撤（%）
    win_rate: float = 0.0                # 胜率（0-1）
    profit_loss_ratio: float = 0.0       # 盈亏比
    total_trades: int = 0                # 总交易次数
    net_return_pct: float = 0.0          # 扣除手续费后净收益（%）
...
class BacktestEngine:
    ...
    def calculate_performance(
        cls,
        results: List[BacktestResultLike],
        config: EvaluationConfig,
    ) -> "PerformanceMetrics":
        """从一批回测结果计算整体绩效指标。"""
        import math

        valid = [r for r in results if r.simulated_return_pct is not None]
        if not valid:
            return PerformanceMetrics(total_trades=len(results))

        # 扣除手续费和滑点后的净收益（每笔双边）
        cost_per_trade = (config.commission_rate + config.slippage_rate) * 2
        net_returns = [
            (r.simulated_return_pct / 100) - cost_per_trade
            for r in valid
        ]
        ...
        net_return_avg = sum(net_returns) / total_trades
        trading_days = config.eval_window_days * total_trades
        annualized = (1 + net_return_avg) ** (252 / max(trading_days, 1)) - 1

        # 夏普比率（无风险利率 2%/年）
        if len(net_returns) > 1:
            import statistics
            std = statistics.stdev(net_returns)
            sharpe = ((annualized - 0.02) / (std * math.sqrt(252))) if std > 0 else 0.0
        else:
            sharpe = 0.0
        ...
        return PerformanceMetrics(
            ...
            net_return_pct=round(net_return_avg * 100, 2),
            total_return_pct=round(sum(net_returns) * 100, 2),
